...
function activeBoosts(p){const now=Date.now(),a=[];if(p.speedBoostUntil>now)a.push({type:'speed',remaining:Math.ceil((p.speedBoostUntil-now)/1000)});if(p.powerBoostUntil>now)a.push({type:'power',remaining:Math.ceil((p.powerBoostUntil-now)/1000)});if(p.shieldUntil>now)a.push({type:'shield',remaining:Math.ceil((p.shieldUntil-now)/1000)});return a}
function pub(p){return{id:p.id,name:p.name,x:p.x,z:p.z,rot:p.rot,inLobby:p.inLobby,level:p.level,xp:p.xp,hp:p.hp,maxHp:p.maxHp,animal:animals[p.level-1][0],power:animals[p.level-1][3],dead:p.dead,spawnProtected:Date.now()<p.protectedUntil,kills:p.kills,coins:p.coins,owned:p.owned,equipped:p.equipped,stamina:p.stamina,maxStamina:p.maxStamina,speedBoost:Date.now()<p.speedBoostUntil,powerBoost:Date.now()<p.powerBoostUntil,shield:Date.now()<p.shieldUntil,activeBoosts:activeBoosts(p)}}
...
function blockedWaterfall(x,z){return Math.abs(x-52)<7.2 && Math.abs(z+35)<5.2}
