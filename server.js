...
function pub(p){return{id:p.id,name:p.name,x:p.x,z:p.z,rot:p.rot,level:p.level,xp:p.xp,hp:p.hp,maxHp:p.maxHp,animal:animals[p.level-1][0],power:animals[p.level-1][3],dead:p.dead,spawnProtected:Date.now()<p.protectedUntil,kills:p.kills,coins:p.coins,owned:p.owned,equipped:p.equipped}}
...
function respawn(p){let s=spawn();p.x=s.x;p.z=s.z;p.rot=s.rot;p.level=1;p.xp=0;p.maxHp=animals[0][2];p.hp=p.maxHp;p.dead=false;p.protectedUntil=Date.now()+5000;p.lastKiller=null}
function hurtPlayer(p,dmg,killer){if(p.dead||Date.now()<p.protectedUntil)return;p.hp-=dmg;p.lastHitBy=killer?.id||null;broadcast({type:'damage',target:p.id,amount:Math.round(dmg),x:p.x,z:p.z});if(p.hp<=0){if(killer){killer.kills++;killer.coins+=10;broadcast({type:'reward',killer:killer.id,coins:10})}die(p,killer?.id)}}
...
app.post('/api/connect',(req,res)=>{
  ...
  let p={id,ws:{readyState:WebSocket.OPEN,send:()=>{}},name:'Joueur',...s,inLobby:true,level:1,xp:0,maxHp:animals[0][2],hp:animals[0][2],dead:false,protectedUntil:Date.now()+3000,lastAttack:0,kills:0,coins:0,owned:[],equipped:null,api:true};
...
app.post('/api/action',(req,res)=>{
  ...
  res.json({ok:true,messages,state:{type:'state',players:active().map(pub),mobs:mobs.map(m=>({id:m.id,level:m.level,x:m.x,z:m.z,rot:m.rot,hp:m.hp,maxHp:m.maxHp,harmless:m.harmless}))}});
});
app.get('/api/state',(req,res)=>{
  const id=apiTokens.get(req.query.token),p=players.get(id); if(!p)return res.status(401).json({error:'session'});
  res.json({type:'state',players:active().map(pub),mobs:mobs.map(m=>({id:m.id,level:m.level,x:m.x,z:m.z,rot:m.rot,hp:m.hp,maxHp:m.maxHp,harmless:m.harmless}))});
});

wss.on('connection',ws=>{
 if(players.size>=MAX_PLAYERS){ws.send(JSON.stringify({type:'full'}));ws.close();return}
 let id=Math.random().toString(36).slice(2,10),s=spawn();
 let p={id,ws,name:'Joueur',...s,inLobby:true,level:1,xp:0,maxHp:animals[0][2],hp:animals[0][2],dead:false,protectedUntil:Date.now()+3000,lastAttack:0,kills:0,coins:0,owned:[],equipped:null};
 players.set(id,p);
 ws.send(JSON.stringify({type:'init',id,world:WORLD,animals,cosmetics,players:[pub(p)],mobs}));
 broadcast({type:'players',players:active().map(pub)});
 function handleMessage(p,m,send){
  if(!p||!m)return;
  if(m.type==='join'){p.name=String(m.name||'Joueur').replace(/[^\p{L}\p{N}_ -]/gu,'').slice(0,16)||'Joueur';p.inLobby=false;p.dead=false;p.protectedUntil=Date.now()+5000;let s=spawn();p.x=s.x;p.z=s.z;p.rot=s.rot;send({type:'players',players:active().map(pub)});broadcast({type:'players',players:active().map(pub)});return}
...
setInterval(()=>{let now=Date.now();for(const m of mobs){if(m.hp<=0)continue;if(m.harmless){m.wander+=(Math.random()-.5)*.12;m.x+=Math.sin(m.wander)*.012;m.z+=Math.cos(m.wander)*.012;continue}let t=null,best=16;for(const p of players.values()){if(p.inLobby||p.dead||now<p.protectedUntil)continue;let d=dist(m,p);if(d<best){best=d;t=p}}if(t){if(best>2.5){let dx=(t.x-m.x)/best,dz=(t.z-m.z)/best,sp=.018+Math.min(m.level,20)*.0018;m.x+=dx*sp;m.z+=dz*sp;m.rot=Math.atan2(dx,dz)}else if(now-m.lastAttack>900){m.lastAttack=now;hurtPlayer(t,animals[m.level-1][3]*.35)}}else{m.wander+=(Math.random()-.5)*.08;m.x+=Math.sin(m.wander)*.01;m.z+=Math.cos(m.wander)*.01}}
 ...
 broadcast({type:'state',players:active().map(pub),mobs:mobs.map(m=>({id:m.id,level:m.level,x:m.x,z:m.z,rot:m.rot,hp:m.hp,maxHp:m.maxHp,harmless:m.harmless}))});
},66);
