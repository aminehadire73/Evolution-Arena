...
const boosts=[]; let nextBoost=1;
const BOOST_TYPES=[['speed','TURBO',0x64d8ff],['damage','DÉGÂTS x2',0xff765f],['shield','BOUCLIER',0xc9a7ff],['heal','RÉGÉNÉRATION',0x83e85b]];
...
// Population volontairement basse : 15 mobs maximum au démarrage.
for(let l=1;l<=10;l++){let n=l<=5?2:1;for(let i=0;i<n;i++)mobs.push(mob(l,l<=2))}
function makeBoost(){
  const a=Math.random()*Math.PI*2,r=18+Math.random()*105;
  const type=BOOST_TYPES[Math.floor(Math.random()*BOOST_TYPES.length)];
  boosts.push({id:'b'+nextBoost++,type:type[0],name:type[1],x:Math.cos(a)*r,z:Math.sin(a)*r,expires:Date.now()+45000});
}
for(let i=0;i<10;i++)makeBoost();
function boostPublic(){return boosts.map(b=>({id:b.id,type:b.type,name:b.name,x:b.x,z:b.z}));}
...
function respawn(p){let s=spawn();p.x=s.x;p.z=s.z;p.rot=s.rot;p.level=1;p.xp=0;p.maxHp=animals[0][2];p.hp=p.maxHp;p.dead=false;p.protectedUntil=Date.now()+5000;p.lastKiller=null;p.stamina=p.maxStamina;p.boost=null}
function hurtPlayer(p,dmg,killer){if(p.dead||Date.now()<p.protectedUntil)return;if(p.boost?.type==='shield')dmg*=0.45;p.hp-=dmg;p.lastHitBy=killer?.id||null;broadcast({type:'damage',target:p.id,amount:Math.round(dmg),x:p.x,z:p.z});if(p.hp<=0){if(killer){killer.kills++;killer.coins+=10;broadcast({type:'reward',killer:killer.id,coins:10})}die(p,killer?.id)}}
...
app.post('/api/connect',(req,res)=>{
  ...
  let p={id,ws:{readyState:WebSocket.OPEN,send:()=>{}},name:'Joueur',...s,inLobby:true,level:1,xp:0,maxHp:animals[0][2],hp:animals[0][2],dead:false,protectedUntil:Date.now()+3000,lastAttack:0,kills:0,coins:0,owned:[],equipped:null,stamina:100,maxStamina:100,api:true};
  players.set(id,p);let token=Math.random().toString(36).slice(2)+Date.now().toString(36);apiTokens.set(token,id);
  res.json({token,type:'init',id,world:WORLD,animals,cosmetics,players:[pub(p)],mobs,boosts:boostPublic()});
});
app.post('/api/action',(req,res)=>{
  ...
  res.json({ok:true,messages,state:{type:'state',players:active().map(pub),mobs:mobs.map(m=>({id:m.id,level:m.level,x:m.x,z:m.z,rot:m.rot,hp:m.hp,maxHp:m.maxHp,harmless:m.harmless})),boosts:boostPublic()}});
});
app.get('/api/state',(req,res)=>{
  const id=apiTokens.get(req.query.token),p=players.get(id); if(!p)return res.status(401).json({error:'session'});
  res.json({type:'state',players:active().map(pub),mobs:mobs.map(m=>({id:m.id,level:m.level,x:m.x,z:m.z,rot:m.rot,hp:m.hp,maxHp:m.maxHp,harmless:m.harmless})),boosts:boostPublic()});
});

wss.on('connection',ws=>{
 if(players.size>=MAX_PLAYERS){ws.send(JSON.stringify({type:'full'}));ws.close();return}
 let id=Math.random().toString(36).slice(2,10),s=spawn();
 let p={id,ws,name:'Joueur',...s,inLobby:true,level:1,xp:0,maxHp:animals[0][2],hp:animals[0][2],dead:false,protectedUntil:Date.now()+3000,lastAttack:0,kills:0,coins:0,owned:[],equipped:null,stamina:100,maxStamina:100};
 players.set(id,p);
 ws.send(JSON.stringify({type:'init',id,world:WORLD,animals,cosmetics,players:[pub(p)],mobs,boosts:boostPublic()}));
 broadcast({type:'players',players:active().map(pub)});
 function handleMessage(p,m,send){
  if(!p||!m)return;
  if(m.type==='join'){p.name=String(m.name||'Joueur').replace(/[^\p{L}\p{N}_ -]/gu,'').slice(0,16)||'Joueur';p.inLobby=false;p.dead=false;p.stamina=p.maxStamina;p.boost=null;p.protectedUntil=Date.now()+5000;let s=spawn();p.x=s.x;p.z=s.z;p.rot=s.rot;send({type:'players',players:active().map(pub)});broadcast({type:'players',players:active().map(pub)});return}
  if(m.type==='lobby'){p.inLobby=true;p.dead=false;p.boost=null;send({type:'lobby',player:pub(p)});broadcast({type:'players',players:active().map(pub)});return}
  if(m.type==='move'&&!p.dead&&!p.inLobby){let turn=Number(m.turn)||0,forward=Number(m.forward)||0;p.rot+=clamp(turn,-1,1)*.075;let sprint=!!m.sprint&&forward>0&&p.stamina>1;if(sprint)p.stamina=Math.max(0,p.stamina-.85);else p.stamina=Math.min(p.maxStamina,p.stamina+.45);let speed=(.10+Math.min(p.level,20)*.002)*(sprint?1.72:1)*(p.boost?.type==='speed'?1.45:1)*clamp(Math.abs(forward),0,1);let nx=clamp(p.x+Math.sin(p.rot)*speed*Math.sign(forward),-WORLD/2+2,WORLD/2-2),nz=clamp(p.z+Math.cos(p.rot)*speed*Math.sign(forward),-WORLD/2+2,WORLD/2-2);if(Math.abs(nx-52)<6&&nz>-40&&nz<-26){}else{p.x=nx;p.z=nz}for(let i=boosts.length-1;i>=0;i--){const b=boosts[i];if(dist(p,b)<2.2){p.boost={type:b.type,name:b.name,until:Date.now()+9000};if(b.type==='heal')p.hp=p.maxHp;boosts.splice(i,1);}}return}
  if(m.type==='attack'&&!p.dead&&!p.inLobby&&Date.now()-p.lastAttack>=550&&Date.now()>=p.protectedUntil){p.lastAttack=Date.now();let reach=3+animals[p.level-1][4]*.5,damage=animals[p.level-1][3]*(p.boost?.type==='damage'?2:1);for(const o of players.values())if(o.id!==p.id&&!o.dead&&Date.now()>=o.protectedUntil&&dist(p,o)<=reach)hurtPlayer(o,damage,p);for(const mm of mobs)if(mm.hp>0&&dist(p,mm)<=reach){mm.hp-=damage;broadcast({type:'damageMob',id:mm.id,amount:Math.round(damage),x:mm.x,z:mm.z});if(mm.hp<=0){gainXp(p,Math.max(1,Math.floor(mm.level*8)));let z=zone(mm.level);mm.x=rand(z[0],z[1]);mm.z=rand(-WORLD/2,WORLD/2);mm.hp=mm.maxHp}}return}
  if(m.type==='respawn'&&p.dead&&!p.inLobby){respawn(p);return}
  if(m.type==='buy'){let item=cosmetics.find(c=>c.id===m.id);if(item&&!p.owned.includes(item.id)&&p.coins>=item.cost){p.coins-=item.cost;p.owned.push(item.id);send({type:'shop',id:p.id,coins:p.coins,owned:p.owned,equipped:p.equipped,stamina:p.stamina,maxStamina:p.maxStamina});}return}
  if(m.type==='equip'){if(m.id===null||p.owned.includes(m.id)){p.equipped=m.id;send({type:'shop',id:p.id,coins:p.coins,owned:p.owned,equipped:p.equipped,stamina:p.stamina,maxStamina:p.maxStamina});}return}
 }
...
setInterval(()=>{let now=Date.now();for(const p of players.values()) if(!p.inLobby&&!p.dead) p.stamina=Math.min(p.maxStamina,p.stamina+.8); for(const m of mobs){if(m.hp<=0)continue;if(m.harmless){m.wander+=(Math.random()-.5)*.12;m.x+=Math.sin(m.wander)*.012;m.z+=Math.cos(m.wander)*.012;continue}let t=null,best=16;for(const p of players.values()){if(p.inLobby||p.dead||now<p.protectedUntil)continue;let d=dist(m,p);if(d<best){best=d;t=p}}if(t){if(best>2.5){let dx=(t.x-m.x)/best,dz=(t.z-m.z)/best,sp=.018+Math.min(m.level,20)*.0018;let nx=m.x+dx*sp,nz=m.z+dz*sp;if(!(Math.abs(nx-52)<7&&nz>-42&&nz<-25)){m.x=nx;m.z=nz}m.rot=Math.atan2(dx,dz)}else if(now-m.lastAttack>900){m.lastAttack=now;hurtPlayer(t,animals[m.level-1][3]*.35)}}else{m.wander+=(Math.random()-.5)*.08;let nx=m.x+Math.sin(m.wander)*.01,nz=m.z+Math.cos(m.wander)*.01;if(!(Math.abs(nx-52)<7&&nz>-42&&nz<-25)){m.x=nx;m.z=nz}}}
 for(let l=1;l<=10;l++){let alive=mobs.filter(m=>m.level===l&&m.hp>0).length,w=l<=5?2:1;for(let i=alive;i<w;i++)mobs.push(mob(l,l<=2))}
 broadcast({type:'state',players:active().map(pub),mobs:mobs.map(m=>({id:m.id,level:m.level,x:m.x,z:m.z,rot:m.rot,hp:m.hp,maxHp:m.maxHp,harmless:m.harmless})),boosts:boostPublic()});
},100);
