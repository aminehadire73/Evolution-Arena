...
function spawn(){return{x:rand(-10,10),z:rand(-10,10),rot:rand(-Math.PI,Math.PI)}}
...
function respawn(p){let s=spawn();p.x=s.x;p.z=s.z;p.rot=s.rot;p.level=1;p.xp=0;p.maxHp=animals[0][2];p.hp=p.maxHp;p.dead=false;p.protectedUntil=Date.now()+3000;p.lastKiller=null}
...
wss.on('connection',ws=>{
 ...
 ws.on('message',raw=>{let m;try{m=JSON.parse(raw)}catch{return};
  if(m.type==='join'){p.name=String(m.name||'Joueur').replace(/[^\p{L}\p{N}_ -]/gu,'').slice(0,16)||'Joueur';p.inLobby=false;p.dead=false;p.protectedUntil=Date.now()+3000;let s=spawn();p.x=s.x;p.z=s.z;p.rot=s.rot;ws.send(JSON.stringify({type:'players',players:active().map(pub)}));broadcast({type:'players',players:active().map(pub)});}
  if(m.type==='lobby'){p.inLobby=true;p.dead=false;ws.send(JSON.stringify({type:'lobby',player:pub(p)}));broadcast({type:'players',players:active().map(pub)});}
  ...
  if(m.type==='respawn'&&p.dead&&!p.inLobby)respawn(p);
  if(m.type==='buy'){let item=cosmetics.find(c=>c.id===m.id);if(item&&!p.owned.includes(item.id)&&p.coins>=item.cost){p.coins-=item.cost;p.owned.push(item.id);p.ws.send(JSON.stringify({type:'shop',id:p.id,coins:p.coins,owned:p.owned,equipped:p.equipped}))}}
  if(m.type==='equip'){if(m.id===null||p.owned.includes(m.id)){p.equipped=m.id;p.ws.send(JSON.stringify({type:'shop',id:p.id,coins:p.coins,owned:p.owned,equipped:p.equipped}))}}
 });
