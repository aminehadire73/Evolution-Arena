...
async function connectApi(name){
  try{const r=await fetch('/api/connect',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});if(!r.ok)throw new Error('api');const d=await r.json();apiToken=d.token;apiMode=true;handleMessage(d);enterGame(name);if(apiPoll)clearInterval(apiPoll);apiPoll=setInterval(()=>{if(!apiToken)return;fetch('/api/state?token='+encodeURIComponent(apiToken)).then(r=>r.json()).then(handleMessage).catch(()=>{})},300);startJungleMusic();$('play').disabled=false;$('play').innerHTML='<span>SPAWN</span><b>↗</b>';return true}catch(e){apiMode=false;return false}
}
...
function handleMessage(m){
  if(m.type==='init'){meId=m.id;animals=m.animals||CLIENT_ANIMALS;cosmetics=m.cosmetics||[];players.clear();m.players.forEach(p=>players.set(p.id,p));m.mobs.forEach(x=>mobs.set(x.id,x));boosts.clear();(m.boosts||[]).forEach(b=>boosts.set(b.id,b));me=players.get(meId);renderShop()}
  if(m.type==='players'){
  ...
  if(m.type==='state'){
    boosts.clear();(m.boosts||[]).forEach(b=>boosts.set(b.id,b));
...
function goLobby(){
  sendMsg({type:'lobby'});
  if(me)me.inLobby=true;me&& (me.dead=false);
...
function initGame(){
  ...
  renderer=new THREE.WebGLRenderer({antialias:false,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(devicePixelRatio,1.1));renderer.setSize(innerWidth,innerHeight);renderer.shadowMap.enabled=false;renderer.domElement.className='gamecanvas';document.body.appendChild(renderer.domElement);cameraYaw=me?.rot||0;cameraPitch=.42;cameraDistance=11;initMinimap();
  scene.add(new THREE.HemisphereLight(0xdfffd0,0x67513a,2.15));startWaterSound();let sun=new THREE.DirectionalLight(0xfff5d8,1.8);sun.position.set(-40,90,30);scene.add(sun);makeWorld();clock=new THREE.Clock();requestAnimationFrame(gameLoop);
}
...
function makeWorld(){
  ...
  for(let i=0;i<110;i++){const x=(Math.random()-.5)*250,z=(Math.random()-.5)*250;if(Math.hypot(x,z)<13)continue;Math.random()<.64?tree(x,z):rock(x,z)}
  for(let i=0;i<85;i++){const x=(Math.random()-.5)*240,z=(Math.random()-.5)*240;if(Math.hypot(x,z)<12)continue;const s=.3+Math.random()*.55;const bush=new THREE.Mesh(new THREE.IcosahedronGeometry(s,0),new THREE.MeshStandardMaterial({color:i%3?0x3d7b37:0x557f35,roughness:1}));bush.position.set(x,surfaceY(x,z)+s*.7,z);scene.add(bush)}
  for(let i=0;i<70;i++){const x=(Math.random()-.5)*235,z=(Math.random()-.5)*235;if(Math.hypot(x,z)<14)continue;const stem=new THREE.Mesh(new THREE.CylinderGeometry(.015,.025,.32,5),new THREE.MeshBasicMaterial({color:0x6e8d3f}));stem.position.set(x,surfaceY(x,z)+.16,z);scene.add(stem);const flower=new THREE.Mesh(new THREE.SphereGeometry(.055,5,4),new THREE.MeshBasicMaterial({color:i%2?0xf0b2d2:0xf0d37b}));flower.position.set(x,surfaceY(x,z)+.35,z);scene.add(flower)}
}
...
function makeWaterfall(x,z){const cliff=new THREE.Mesh(new THREE.BoxGeometry(14,5,8),new THREE.MeshStandardMaterial({color:0x625743,roughness:1}));cliff.position.set(x,2.2,z);scene.add(cliff);const fall=new THREE.Mesh(new THREE.PlaneGeometry(7,6),new THREE.MeshBasicMaterial({color:0x75d3e9,transparent:true,opacity:.72,side:THREE.DoubleSide}));fall.position.set(x,3.1,z+3.8);scene.add(fall);for(let i=0;i<42;i++){const d=new THREE.Mesh(new THREE.SphereGeometry(.045+Math.random()*.04,5,4),new THREE.MeshBasicMaterial({color:0xb9f3ff,transparent:true,opacity:.65}));d.position.set(x+rand(-3.2,3.2),rand(.2,4.8),z+rand(2.5,5.2));d.userData.baseY=d.position.y;d.userData.speed=.7+Math.random()*1.5;scene.add(d);waterDrops.push(d)}}
...
function animalMesh(lvl,player=false){
  const d=animalData(lvl),s=Math.min(2.8,.75+(lvl-1)*.105),c=colors[lvl-1]||0x777777,g=new THREE.Group();
  ...
  // Insect / worm
  if(lvl===1){g.userData.wormSegments=[];for(let i=0;i<7;i++){const seg=part(g,new THREE.SphereGeometry(i===6?.34*s:.30*s,8,6),fur,0,.38*s,(i-3)*.34*s,1.15,.72,1.35);g.userData.wormSegments.push(seg)}g.userData.modelHeight=.75*s;}
    ...
    g.userData.legs=[];for(const x of [-.48,.48])for(const z of [-.65,.72]){const leg=part(g,new THREE.CylinderGeometry(.14*s,.19*s,.9*s,6),fur,x*s,.47*s,z*s,legScale,1,legScale);leg.userData.phase=(z>0?0:Math.PI)+(x>0?.15:0);g.userData.legs.push(leg);part(g,new THREE.SphereGeometry(.16*s,6,5),dark,x*s,.08*s,z*s,1,.6,1)}
    // ears vary by animal
...
function applyCosmetic(obj,id){if(!obj||obj.userData.cosmetic===id)return;let old=obj.getObjectByName('cosmetic');if(old)obj.remove(old);obj.userData.cosmetic=id;if(!id)return;let lvl=obj.userData.level||1,s=animalData(lvl)?.[4]||1,g=new THREE.Group();g.name='cosmetic';
 ...
 obj.add(g)}
function animateAnimal(m,moving,now,phaseOffset=0){
  const phase=now*.012+phaseOffset;
  if(m.userData.wormSegments){m.userData.wormSegments.forEach((seg,i)=>{seg.position.y=.38+Math.sin(phase*1.8+i*.8)*.06;seg.position.z=(i-3)*.34+Math.sin(phase*1.8+i*.55)*.045;seg.position.x=Math.sin(phase*1.8+i*.65)*.16;seg.rotation.y=Math.sin(phase*1.8+i*.65)*.28;});}
  if(m.userData.legs){m.userData.legs.forEach(leg=>{const swing=moving?Math.sin(phase*1.9+leg.userData.phase)*.38:0;leg.rotation.x=swing;});}
}
function sync(){
  const now=performance.now();
  for(const [id,p] of players){
    let m=meshes.get(id);
    if(!m||m.userData.level!==p.level){if(m)scene.remove(m);m=animalMesh(p.level,true);scene.add(m);meshes.set(id,m);m.position.set(p.x,surfaceY(p.x,p.z)+.35,p.z)}
    const tx=p.x,tz=p.z,dx=tx-m.position.x,dz=tz-m.position.z,moving=Math.hypot(dx,dz)>.015;
    m.position.x += dx*.32;m.position.z += dz*.32;m.position.y=surfaceY(m.position.x,m.position.z)+.35;
    m.rotation.y += Math.atan2(Math.sin(p.rot-m.rotation.y),Math.cos(p.rot-m.rotation.y))*.35;
    m.visible=!p.dead;healthBar(m,p.hp,p.maxHp);applyCosmetic(m,p.equipped);animateAnimal(m,moving,now,id.length);
  }
  for(const [id,x] of mobs){
    let m=mobMeshes.get(id);if(!m){m=animalMesh(x.level);scene.add(m);m.position.set(x.x,surfaceY(x.x,x.z)+.35,x.z)}
    const dx=x.x-m.position.x,dz=x.z-m.position.z,moving=Math.hypot(dx,dz)>.012;
    m.position.x += dx*.28;m.position.z += dz*.28;m.position.y=surfaceY(m.position.x,m.position.z)+.35;
    m.rotation.y += Math.atan2(Math.sin(x.rot-m.rotation.y),Math.cos(x.rot-m.rotation.y))*.3;m.visible=x.hp>0;healthBar(m,x.hp,x.maxHp);animateAnimal(m,moving,now,id.length);
  }
  if(typeof boosts!=='undefined'){
    for(const b of boosts.values()){let bm=boostMeshes.get(b.id);if(!bm){bm=new THREE.Group();const core=new THREE.Mesh(new THREE.OctahedronGeometry(.45,0),new THREE.MeshStandardMaterial({color:b.type==='speed'?0x63d9ff:b.type==='damage'?0xff6b5b:b.type==='shield'?0xc8a4ff:0x7be36c,emissive:b.type==='speed'?0x164a66:0x173f18,emissiveIntensity:.6}));bm.add(core);bm.userData.type=b.type;scene.add(bm);boostMeshes.set(b.id,bm)}bm.position.set(b.x,surfaceY(b.x,b.z)+.75,b.z);bm.rotation.y=now*.0018;bm.position.y+=Math.sin(now*.004+b.x)*.12;}
    for(const [id,bm] of boostMeshes)if(!boosts.has(id)){scene.remove(bm);boostMeshes.delete(id)}
  }
}
let last=0;function gameLoop(t){requestAnimationFrame(gameLoop);if(!renderer)return;if(t-last>75){sendMove();last=t}waterDrops.forEach(d=>{d.position.y-=d.userData.speed*.018;if(d.position.y<.15)d.position.y=d.userData.baseY});sync();if(me){let m=meshes.get(meId);if(m){const s=animalData(me.level)[4],targetDist=cameraDistance+s*.8,cy=Math.cos(cameraPitch),sy=Math.sin(cameraPitch);let target=new THREE.Vector3(m.position.x+Math.sin(cameraYaw)*cy*targetDist,m.position.y+sy*targetDist+2.2,m.position.z+Math.cos(cameraYaw)*cy*targetDist);camera.position.lerp(target,.16);camera.lookAt(m.position.x,m.position.y+.8*s,m.position.z)}hudUpdate()}renderer.render(scene,camera)}
...
function startWaterSound(){try{if(!audio)return;const len=audio.sampleRate*2,buf=audio.createBuffer(1,len,audio.sampleRate),data=buf.getChannelData(0);for(let i=0;i<len;i++)data[i]=(Math.random()*2-1)*.32;const src=audio.createBufferSource();src.buffer=buf;src.loop=true;const filter=audio.createBiquadFilter();filter.type='lowpass';filter.frequency.value=1500;const g=audio.createGain();g.gain.value=.006;src.connect(filter).connect(g).connect(audio.destination);src.start()}catch{}}
...
window.addEventListener('pointerdown',e=>{if(e.button===2&&gameStarted){cameraDragging=true;lastPointerX=e.clientX;lastPointerY=e.clientY;document.body.classList.add('camera-drag')}});
window.addEventListener('pointermove',e=>{if(!cameraDragging)return;cameraYaw-=(e.clientX-lastPointerX)*.006;cameraPitch=THREE.MathUtils.clamp(cameraPitch-(e.clientY-lastPointerY)*.004,.12,1.05);lastPointerX=e.clientX;lastPointerY=e.clientY});
window.addEventListener('pointerup',e=>{if(e.button===2){cameraDragging=false;document.body.classList.remove('camera-drag')}});
window.addEventListener('pointercancel',()=>{cameraDragging=false});
